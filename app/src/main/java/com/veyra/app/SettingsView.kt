...
class SettingsView(private val activity: MainActivity, private val onBack: () -> Unit) : View(activity) {
 ...
 private val rows=listOf("Daily reminder" to "Choose when Veyra reminds you to take one small action.","Nudges" to "Small recurring reminders like water, stretch and rest.","Manage habits" to "Rename or remove your habits.","Habit history" to "See your recent completion history.","Achievements" to "View your unlocked milestones.","Backup data" to "Export your Veyra data as a backup file.","Restore backup" to "Restore data from a Veyra backup.","About Veyra" to "Version 1.2 and information about the app.","Reset Veyra data" to "Clear all locally stored Veyra progress.")
 ...
 override fun onTouchEvent(e:MotionEvent):Boolean{if(e.action!=MotionEvent.ACTION_UP)return true;val x=e.x/density;val y=(e.y-topInset)/density;val h=(height-topInset-bottomInset)/density;if((x<70f&&y<80f)||y>h-58f){onBack();return true};val i=((y-94f)/73f).toInt();if(i in rows.indices&&y>=88f&&y<94f+rows.size*73f)activity.settingsAction(i);return true}
}
