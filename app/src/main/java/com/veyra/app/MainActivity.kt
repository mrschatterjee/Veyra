...
class MainActivity : Activity() {
    ...
    override fun onRequestPermissionsResult(requestCode:Int,permissions:Array<out String>,grantResults:IntArray){super.onRequestPermissionsResult(requestCode,permissions,grantResults);if(requestCode==notificationPermission){pendingReminder?.let{if(grantResults.firstOrNull()==PackageManager.PERMISSION_GRANTED)HabitReminderStore(this).save(it);pendingReminder=null;showSettings()}}}
    ...
    private fun confirmReset(){VeyraGlassDialog.showConfirm(this,"Reset Veyra?","All habits, progress, goals, mood, journal entries and achievements will be removed from Veyra.","RESET"){store.reset();HabitReminderStore(this).all().forEach{HabitReminderScheduler.cancel(this,it.habitId)};showHome()}}
