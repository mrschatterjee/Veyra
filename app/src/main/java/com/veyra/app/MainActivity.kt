...
class MainActivity : Activity() {
    ...
    fun showSettings(){val labels=arrayOf("Daily reminder","Manage habits","Habit history","Achievements","Backup data","Restore backup","About Veyra • v1.2","Reset Veyra data");AlertDialog.Builder(this).setTitle("Veyra settings").setItems(labels){_,which->when(which){0->reminderDialog();1->manageHabits();2->showHabitHistory();3->showAchievements();4->createBackupFile();5->openBackupFile();6->showAbout();7->confirmReset()}}.show()}
    private fun showAchievements(){val items=store.achievements().map{if(it.unlocked)"✓ ${it.title}\n${it.description}" else "○ ${it.title}\n${it.description}"}.toTypedArray();AlertDialog.Builder(this).setTitle("Achievements").setItems(items,null).setPositiveButton("OK",null).show()}
    private fun showHabitHistory(){val habits=store.habits();if(habits.isEmpty()){AlertDialog.Builder(this).setTitle("Habit history").setMessage("Add a habit first to build completion history.").setPositiveButton("OK",null).show();return};val date=Calendar.getInstance();val f=SimpleDateFormat("EEE, d MMM",Locale.getDefault());val lines=mutableListOf<String>();habits.forEach{habit->lines+="${habit.name}";val days=mutableListOf<String>();repeat(14){val key=SimpleDateFormat("yyyy-MM-dd",Locale.US).format(date.time);days+="${if(store.isCompleted(habit.id,key))"✓" else "—"} ${f.format(date.time)}";date.add(Calendar.DAY_OF_YEAR,-1)};lines+=days.chunked(2).map{it.joinToString("    ")}.map{"  $it"};date.timeInMillis=Calendar.getInstance().timeInMillis;lines+=""};AlertDialog.Builder(this).setTitle("Habit history • 14 days").setMessage(lines.joinToString("\n")).setPositiveButton("OK",null).show()}
    ...
    private fun manageHabits(){val habits=store.habits();if(habits.isEmpty()){AlertDialog.Builder(this).setTitle("Manage habits").setMessage("You have no habits yet. Add one from the Habits tab.").setPositiveButton("OK",null).show();return};AlertDialog.Builder(this).setTitle("Manage habits").setItems(habits.map{"${it.name}\nTap to edit or delete"}.toTypedArray()){_,index->habitActions(habits[index].id)}.show()}
    private fun habitActions(id:Long){val habit=store.habits().firstOrNull{it.id==id}?:return;AlertDialog.Builder(this).setTitle(habit.name).setItems(arrayOf("Rename","Delete")){_,which->if(which==0)renameHabit(habit)else deleteHabit(habit)}.show()}
    ...
    private class OpeningView(context:Context,val done:()->Unit):View(context){
        private val paint=Paint(Paint.ANTI_ALIAS_FLAG);private val stars=List(70){Random(it*19L+4L).nextFloat() to Random(it*37L+9L).nextFloat()};private val handler=Handler(Looper.getMainLooper());private var phase=0;private var topInset=0;private var bottomInset=0
        init{setOnApplyWindowInsetsListener{_,insets->topInset=insets.getInsets(WindowInsets.Type.statusBars()).top;bottomInset=insets.getInsets(WindowInsets.Type.navigationBars()).bottom;invalidate();insets};postDelayed({phase=1;invalidate()},900);postDelayed({phase=2;invalidate()},1800);postDelayed({done()},2700)}
        override fun onDetachedFromWindow(){handler.removeCallbacksAndMessages(null);super.onDetachedFromWindow()}
        override fun onDraw(c:Canvas){val w=width.toFloat();val top=topInset.toFloat();val h=(height-topInset-bottomInset).toFloat();c.save();c.clipRect(0f,top,w,top+h);c.translate(0f,top);paint.shader=LinearGradient(0f,0f,w,h,Color.rgb(9,5,29),Color.rgb(65,31,122),Shader.TileMode.CLAMP);c.drawRect(0f,0f,w,h,paint);paint.shader=null;paint.color=Color.argb(110,200,170,255);stars.forEach{(x,y)->c.drawCircle(x*w,y*h,if(x>.7f)2.5f else 1.4f,paint)};paint.textAlign=Paint.Align.CENTER;paint.typeface=Typeface.create("sans",Typeface.BOLD);paint.color=Color.WHITE;paint.textSize=48f;val title=when(phase){0->"VEYRA";1->"GOOD MORNING";else->"BUILD YOUR UNIVERSE"};c.drawText(title,w/2f,h/2f,paint);paint.textSize=18f;paint.typeface=Typeface.create("sans",Typeface.NORMAL);paint.color=Color.argb(190,255,255,255);val sub=when(phase){0->"";1->"One small action at a time.";else->""};c.drawText(sub,w/2f,h/2f+34f,paint);paint.textAlign=Paint.Align.LEFT;c.restore()}
    }
}
