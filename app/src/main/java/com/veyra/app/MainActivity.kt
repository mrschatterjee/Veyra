...
class MainActivity : Activity() {
    ...
    fun showHabitReminderFromOverview(habitId:Long){showHabitReminder(habitId)}
    fun saveHabitReminder(id:Long,name:String,hour:Int,minute:Int,enabled:Boolean){val item=HabitReminder(id,name,hour,minute,enabled);if(enabled&&Build.VERSION.SDK_INT>=33&&checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED){pendingReminder=item;requestNotificationPermission();return};HabitReminderStore(this).save(item);HabitReminderScheduler.schedule(this,item);if(enabled)NotificationReliability.promptIfNeeded(this);showReminderPage()}
    private fun renameHabit(habit:VeyraStore.HabitRecord){textInput("Rename habit","Enter a new name"){name->store.renameHabit(habit.id,name);HabitReminderStore(this).get(habit.id)?.let{HabitReminderStore(this).save(it.copy(habitName=name))};showHabitsPage()}}
    private fun deleteHabit(habit:VeyraStore.HabitRecord){VeyraGlassDialog.showConfirm(this,"Delete habit?","\"${habit.name}\" and its completion history will be removed from Veyra.","DELETE"){HabitReminderStore(this).remove(habit.id);HabitReminderScheduler.cancel(this,habit.id);store.removeHabit(habit.id);showHabitsPage()}}
    ...
    private fun showReminderPage(){val view=HabitRemindersView(this);setContentView(view);VeyraMotion.enter(view,18f)}
    ...
    override fun onRequestPermissionsResult(requestCode:Int,permissions:Array<out String>,grantResults:IntArray){super.onRequestPermissionsResult(requestCode,permissions,grantResults);if(requestCode==notificationPermission){val pending=pendingReminder;pendingReminder=null;if(pending!=null){if(grantResults.firstOrNull()==PackageManager.PERMISSION_GRANTED){HabitReminderStore(this).save(pending);HabitReminderScheduler.schedule(this,pending);NotificationReliability.promptIfNeeded(this)}else VeyraGlassDialog.showInfo(this,"Notifications are off","Veyra needs notification permission to send habit reminders. You can allow notifications later in Android Settings.")};if(pending!=null)showReminderPage()}}
