...
class MainActivity : Activity() {
    ...
    private fun deleteHabit(habit:VeyraStore.HabitRecord){VeyraGlassDialog.showConfirm(this,"Delete habit?","\"${habit.name}\" and its completion history will be removed from Veyra.","DELETE"){HabitReminderStore(this).remove(habit.id);store.removeHabit(habit.id);showHabitsPage()}}
    ...
    override fun onRequestPermissionsResult(requestCode:Int,permissions:Array<out String>,grantResults:IntArray){super.onRequestPermissionsResult(requestCode,permissions,grantResults);if(requestCode==notificationPermission){val pending=pendingReminder;pendingReminder=null;if(pending!=null){if(grantResults.firstOrNull()==PackageManager.PERMISSION_GRANTED)HabitReminderStore(this).save(pending)else VeyraGlassDialog.showInfo(this,"Notifications are off","Veyra needs notification permission to send habit reminders. You can allow notifications later in Android Settings.")};if(pending!=null)showReminderPage()}}
