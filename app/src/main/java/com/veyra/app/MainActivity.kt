...
class MainActivity : Activity() {
    ...
    override fun onRequestPermissionsResult(requestCode:Int,permissions:Array<out String>,grantResults:IntArray){super.onRequestPermissionsResult(requestCode,permissions,grantResults);if(requestCode==notificationPermission){val pending=pendingReminder;pendingReminder=null;if(pending!=null){if(grantResults.firstOrNull()==PackageManager.PERMISSION_GRANTED){HabitReminderStore(this).save(pending);HabitReminderScheduler.schedule(this,pending)}else VeyraGlassDialog.showInfo(this,"Notifications are off","Veyra needs notification permission to send habit reminders. You can allow notifications later in Android Settings.")};if(pending!=null)showReminderPage()}}
