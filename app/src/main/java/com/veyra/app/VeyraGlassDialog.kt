...
object VeyraGlassDialog {
    ...
    private fun background(): GradientDrawable = GradientDrawable(
        ...
        intArrayOf(Color.rgb(28, 15, 64), Color.rgb(88, 43, 156))
    ...
    private fun button(context: Context, label: String): TextView = text(context, label, 13f, 255, true).apply {
        gravity = Gravity.CENTER
        background = GradientDrawable().apply {
            setColor(Color.argb(48, 255, 255, 255))
            cornerRadius = dp(context, 16).toFloat()
            setStroke(dp(context, 1), Color.argb(75, 225, 210, 255))
        }
    }
    ...
    private fun open(context: Context, title: String, message: String, editor: EditText? = null, buttons: List<Pair<String, () -> Unit>>): Dialog {
        ...
        if (editor != null) {
            ...
            editor.setHintTextColor(Color.argb(125, 255, 255, 255))
            editor.background = GradientDrawable().apply {
                setColor(Color.argb(38, 255, 255, 255)); cornerRadius = dp(context, 16).toFloat()
                setStroke(dp(context, 1), Color.argb(70, 225, 210, 255))
            }
        ...
        buttons.forEachIndexed { index, pair ->
            val b = button(context, pair.first)
            b.setOnClickListener { pair.second(); dialog.dismiss() }
        ...
        dialog.window?.setDimAmount(0.76f)
        dialog.setCanceledOnTouchOutside(true)
        dialog.setOnShowListener {
            dialog.window?.setLayout((context.resources.displayMetrics.widthPixels * 0.88f).toInt(), WindowManager.LayoutParams.WRAP_CONTENT)
        }
        return dialog
    }

    fun showInfo(context: Context, title: String, message: String) {
        open(context, title, message, buttons = listOf("OK" to {})).show()
    }

    fun showConfirm(context: Context, title: String, message: String, confirm: String, onConfirm: () -> Unit) {
        open(context, title, message, buttons = listOf("CANCEL" to {}, confirm to onConfirm)).show()
    }

    fun showChoice(context: Context, title: String, message: String, first: String, second: String, onFirst: () -> Unit, onSecond: () -> Unit) {
        open(context, title, message, buttons = listOf(first to onFirst, second to onSecond)).show()
    }
    ...
    fun showInput(context: Context, title: String, hint: String, value: String = "", onSave: (String) -> Unit) {
        ...
        open(context, title, "", input, listOf("CANCEL" to {}, "SAVE" to { input.text.toString().trim().takeIf { it.isNotEmpty() }?.let(onSave) })).show()
    }
}
